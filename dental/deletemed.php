...
<div id="container">
    <div id="content"><!--Start of content for delete page-->
        <h2 style="margin-left: 45%;">ลบข้อมูลยา</h2>
        <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($_POST['sure'] == 'ยืนยัน') { // Delete the record
                // Make the query
                $q = "DELETE FROM adminmed WHERE medcode='$id' LIMIT 1";
                ...
                if (mysqli_affected_rows($dbcon) == 1) { // If there was no problem
                    ...
                    echo '<p class="error">เกิดข้อผิดพลาดของระบบ ทำให้ไม่สามารถลบข้อมูลได้ ขออภัยในความไม่สะดวก</p>'; // Display error message
            ...
            $q = "SELECT medcode, medname FROM adminmed WHERE medcode='$id'";
            $result = @mysqli_query($dbcon, $q);
            if (mysqli_num_rows($result) == 1) { // Valid user ID, show the form
                // Get the member's data
                $row = mysqli_fetch_array($result, MYSQLI_NUM);
                // Display the name of the member being deleted
                echo "<center><h3>คุณแน่ใจว่าต้องการลบข้อมูลถาวรของ $row[1] (รหัสยา: $row[0]) หรือไม่?</h3></center>";
                // Display the delete page
                echo '<form action="deletemed.php" method="post" style="margin-left: 45%;"> 
